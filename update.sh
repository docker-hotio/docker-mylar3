...
if [[ ${1} == "screenshot" ]]; then
    SERVICE_IP="http://$(ping -c1 -4 service | sed -nE 's/^PING[^(]+\(([^)]+)\).*/\1/p'):8090"
    NETWORK_IDLE="0"
...
EOF
    ...
    version=$(curl -u "${GITHUB_ACTOR}:${GITHUB_TOKEN}" -fsSL "https://api.github.com/repos/mylar3/mylar3/releases/latest" | jq -r .tag_name | sed s/v//g)
    [[ -z ${version} ]] && exit 1
    find . -type f -name '*.Dockerfile' -exec sed -i "s/ARG MYLAR3_VERSION=.*$/ARG MYLAR3_VERSION=${version}/g" {} \;
